...
def parse_transparency(text):
    ...
    """
    match = re.match(r"^([0-9A-Fa-f]+):(0[0-9][0-9]|1[0-1][0-9]|12[0-7]|[0-9][0-9])$", text)
    if match:
...
def parse_color(text):
    ...
    single_hex_map = {
        "0": (0, 0, 0), "1": (104, 104, 104), "2": (0, 18, 144), "3": (0, 39, 251),
        "4": (0, 143, 21), "5": (0, 249, 44), "6": (0, 144, 146), "7": (0, 252, 254),
        "8": (155, 23, 8), "9": (255, 48, 22), "A": (154, 32, 145), "B": (255, 63, 252),
        "C": (148, 145, 25), "D": (255, 253, 51), "E": (184, 184, 184), "F": (255, 255, 255)
    }
    ...
    match = re.match(r"^([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})$", text)
    if match:
        return tuple(int(match.group(i), 16) for i in range(1, 4))

    # Check for octal format (000–377 for each channel)
    match = re.match(r"(000[0-9]|00[1-9][0-9]|01[0-9][0-9]|02[0-9][0-9]|03[0-6][0-9]|037[0-7])"
    ...
    match = re.match(r"(00[0-9]|0[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])"
...
def draw_image_from_text(text, imgtype="png", outputimage=True, resize=1, resizetype="nearest", outfile=None):
    """
    ...
    for count_y, line in enumerate(text_lines):
        ...
        for count_x, pixel in enumerate(pixels):
            ...
            txt_img.line((count_x, count_y, count_x, count_y), fill=(color[0], color[1], color[2], Transparency))
    ...
    if resize > 1:
        new_txt_img = pre_txt_img.resize((num_x * resize, num_y * resize), resizetype_map.get(resizetype, Image.NEAREST))
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--resize", default=1, help="Enter number to resize image", type=int)
    parser.add_argument("-r", "--resizetype", default="nearest", help="Enter resize type")
    parser.add_argument("-v", "--version", action="version", version="1.0.0 RC 1")

    getargs = parser.parse_args()

    with open(getargs.input, "r") as ftest:
        draw_image_from_text(ftest.read(), getargs.outputtype, True, getargs.resize, getargs.resizetype, getargs.output)
